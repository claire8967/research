...
extern double RRM_counter;
extern int RRM_light_right_count;
...
extern uint64_t swap_space_access_count;
...
extern std::vector<int> ECC_counter;
extern std::vector<int> space;

extern std::vector<std::vector<int>> RRM_vector;
// memory_id -> {RRM_counter,hot/cold} 
...
void RRM ( int victim_process_id, int victim_page_id, int page_in_swap_space_id );

int select_swap_space_victim_page ();
int set_page_state( int process_id, int page_id );; // 1:ReadHot 2:ReadCold 3:WriteHot
void change_page_state( int process_id, int page_id, int state, int page_in_swap_space_id ); // 1:short zone 2:medium zone 3: long zone
